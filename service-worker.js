...
let activeMetaMemo=null,activeMetaMemoAt=0,activeMetaPending=null;
...
async function writeMeta(meta){const c=await caches.open(META_CACHE);await c.put(META_URL,new Response(JSON.stringify(meta),{headers:{'Content-Type':'application/json','X-KC-DP-Engine':ENGINE}}));activeMetaMemo=meta;activeMetaMemoAt=Date.now();return meta;}
...
function canonicalVerifiedBuffer(file,buffer){const clean=String(file.installPath||file.path||'').split(/[?#]/)[0].toLowerCase(),dot=clean.lastIndexOf('.'),ext=dot>=0?clean.slice(dot+1):'';if(!CANONICAL_TEXT_EXTENSIONS.has(ext))return buffer;return new TextEncoder().encode(new TextDecoder().decode(buffer).replace(/\r\n?/g,'\n')).buffer;}
...
async function verifiedResponse(file,response){const buffer=await response.arrayBuffer(),verifiedBuffer=canonicalVerifiedBuffer(file,buffer);if(Number(file.bytes||0)>0&&Math.abs(verifiedBuffer.byteLength-Number(file.bytes))>4)throw new Error(`${file.installPath||file.path}: Dateigröße stimmt nicht`);if(file.sha256){const h=await sha256Hex(verifiedBuffer);if(h.toLowerCase()!==String(file.sha256).toLowerCase())throw new Error(`${file.installPath||file.path}: SHA-256 stimmt nicht`);}const headers=new Headers(response.headers);headers.set('X-KC-DP-Verified','1');if(file.sha256)headers.set('X-KC-DP-SHA256',String(file.sha256));return new Response(buffer,{status:200,statusText:'OK',headers});}
async function mapLimit(items,limit,worker){let next=0;const runners=Array.from({length:Math.min(limit,items.length)},async()=>{while(next<items.length){const index=next++;await worker(items[index],index);}});await Promise.all(runners);}
async function cacheRelease(m){const cacheName=m.cacheName||`kc-dp-release-${m.version}`,cache=await caches.open(cacheName),files=m.files.filter(x=>x.runtime!==false);await mapLimit(files,8,async f=>{const source=new URL(f.downloadPath||f.path,self.registration.scope).toString(),target=new URL(f.installPath||f.path,self.registration.scope).toString(),hit=await cache.match(target,{ignoreSearch:true});if(hit)return;const r=await fetchWithTimeout(source,{cache:'no-store'});if(!r.ok)throw new Error(`${f.installPath||f.path}: HTTP ${r.status}`);await cache.put(target,await verifiedResponse(f,r));});return cacheName;}
...
async function activeMeta(){if(activeMetaMemo&&Date.now()-activeMetaMemoAt<5000)return activeMetaMemo;if(activeMetaPending)return activeMetaPending;activeMetaPending=(async()=>{const meta=await maybeRollback((await readMeta())||await ensureInitialRelease());activeMetaMemo=meta;activeMetaMemoAt=Date.now();return meta})();try{return await activeMetaPending}finally{activeMetaPending=null}}
...
async function verifyManifestRelease(m,run){
  ...
  await mapLimit(files,8,async f=>{
    ...
    if(hit){
      try{
        if(f.sha256){const b=await hit.clone().arrayBuffer(),v=canonicalVerifiedBuffer(f,b),h=await sha256Hex(v);valid=h.toLowerCase()===String(f.sha256).toLowerCase()&&(!Number(f.bytes||0)||Math.abs(v.byteLength-Number(f.bytes))<=4);}else if(Number(f.bytes||0)>0){const b=await hit.clone().arrayBuffer(),v=canonicalVerifiedBuffer(f,b);valid=Math.abs(v.byteLength-Number(f.bytes))<=4;}else valid=true;
      }catch(_){valid=false;}
    }
    if(!valid){
...
function badgeHtml(run){
  ...
  return `<div id="kcStartGuardBadge" style="position:fixed;right:10px;bottom:10px;z-index:2147483600;font-family:system-ui,Arial,sans-serif"><button id="kcStartGuardBtn" type="button" style="border:0;border-radius:999px;padding:9px 13px;background:${bg};color:#fff;font-weight:800;box-shadow:0 3px 12px #0004">Start ${label}</button><pre id="kcStartGuardDetails" style="display:none;white-space:pre-wrap;width:min(88vw,560px);max-height:45vh;overflow:auto;margin:7px 0 0;padding:12px;border-radius:12px;background:#fff;color:#222;border:2px solid ${bg};box-shadow:0 8px 26px #0005;font:12px/1.45 ui-monospace,monospace">${details}</pre></div><script>(function(){var b=document.getElementById('kcStartGuardBtn'),d=document.getElementById('kcStartGuardDetails');if(b&&d)b.addEventListener('click',function(){d.style.display=d.style.display==='none'?'block':'none'});if(navigator.serviceWorker)navigator.serviceWorker.addEventListener('message',function(e){var r=e.data&&e.data.type==='KC_DP_START_GUARD_STATUS'&&e.data.run;if(!r||!b||!d)return;var green=r.status==='green',yellow=r.status==='yellow',color=green?'#176b3a':yellow?'#9a6a00':'#a31724';b.textContent='Start '+(green?'✓ GRÜN':yellow?'⚠ GELB':'✕ ROT');b.style.background=color;d.style.borderColor=color;d.textContent=(r.steps||[]).map(function(s){return(s.status==='green'?'✓':s.status==='yellow'?'⚠':'✕')+' '+s.name+': '+s.detail}).join('\\n')})})();</script>`;
}
...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'){
    const guard=new Promise(resolve=>setTimeout(resolve,2500)).then(runStartGuard);
    event.waitUntil(guard);
    event.respondWith((async()=>{
      ...
      return injectBadge(r,{status:'yellow',steps:[{status:'yellow',name:'Startprüfung',detail:'Verifizierter Programmstand geladen; Hintergrundprüfung läuft'}]});
    })());return;
  }
