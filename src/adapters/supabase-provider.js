(function(){
 ...
 async function memberProvisioningTargets(){await ensureSession();const c=validateConfig();const m=await currentMembership();if(m.role!=='admin')throw new Error('Bitte mit deinem normalen Adminzugang anmelden.');const {data}=await api('/rest/v1/kc_core_people?select=person_id,display_name,email,active&org_id=eq.'+encodeURIComponent(c.orgId)+'&active=is.true',{method:'GET'});return data||[];}
