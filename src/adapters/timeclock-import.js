(function(){
  ...
  const aliases={
    memberNo:['memberno','mitgliedsnummer','mitgliedsnr','ausweisnummer','kcnummer','credential','membernumber'],
  ...
  // EINDEUTIGE ZUORDNUNG ueber die Mitgliedsnummer. Sie ist der eine Schluessel, den KC Core,
  // der PC-Manager, der Ausweis-QR und der Dienstplan gemeinsam fuehren. Trifft sie, wird
  // NICHT mehr ueber den Namen geraten - der Name dient dann nur noch der Anzeige.
  const normNr=v=>String(v??'').trim().toUpperCase().replace(/[^A-Z0-9]/g,'');
  function personByMemberNo(nummer){const n=normNr(nummer);if(!n)return null;const rows=K.people.filter(p=>normNr(p.memberNo)===n);return rows.length===1?rows[0]:null;}
  ...
  K.timeclockImport={version:'0.8.0',setProvider(fn){provider=typeof fn==='function'?fn:null;},hasProvider(){return !!provider;},parseDate,parseTime,normalizeRow,parseCsv,parseJson,analyzeFile,pullFromTimeClock};
})();
