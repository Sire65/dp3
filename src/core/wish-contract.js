(function(){
  ...
  function validate(input={}){const row=normalize(input),issues=[],day=dayFor(row.date);if(!TYPES[row.wishType])issues.push({level:'error',code:'wish.type',text:'Unbekannter Wunschstatus.'});if(!day)issues.push({level:'error',code:'wish.date',text:'Datum gehört nicht zum aktuellen Planzeitraum.'});if(!Number.isFinite(row.start)||!Number.isFinite(row.end)||row.end<=row.start)issues.push({level:'error',code:'wish.time',text:'Ende muss nach Beginn liegen.'});if(!['V','H','B','Z'].includes(row.wishZone))issues.push({level:'error',code:'wish.zone',text:'Einsatzbereich muss V, H, B oder Z (Vorbereitung außerhalb) sein.'});if(row.scope==='day'&&row.wishType!=='unavailable')issues.push({level:'error',code:'wish.scope',text:'Ein vollständiger Sperrtag muss „Nicht verfügbar“ sein.'});return issues}
