...
const label=w=>w.wishType==='unavailable'?(w.scope==='day'?'Sperrtag':'Sperrzeit'):({available:'Kannzeit',preferred:'Wunschzeit',if_needed:'Kannzeit · nur wenn nötig'}[w.wishType]||w.wishType);
const zone=w=>({V:'Vorne',H:'Hinten',B:'Beides (V/H)',Z:'Vorbereitung außerhalb'}[w.wishZone||'B']);
const summary=list=>list.length?`<div class="mm-summary">${list.map(w=>`<span class="mm-chip ${esc(w.wishType)}">${w.scope==='day'?'ganztägig':`${tm(w.start)}–${tm(w.end)}`} <b>(${esc(label(w))})</b>${w.wishType!=='unavailable'?` · ${zone(w)}`:''}</span>`).join('')}</div>`:'<p class="mm-empty">Noch keine Angaben für diesen Tag.</p>';
