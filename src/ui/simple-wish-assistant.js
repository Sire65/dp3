...
function teamGraphic(value){
 ...
 const activePeople=(K.people||[]).filter(p=>p.active!==false),blocked=activePeople.filter(p=>dayBlocked(p.personId)&&p.personId===self()),locked=activePeople.filter(p=>p.personId!==self()&&!['can','wish','standby'].some(k=>copyAllowed(p.personId,k))),people=activePeople.filter(p=>!dayBlocked(p.personId)&&(p.personId===self()||!locked.includes(p))).map(p=>{
...
function copyAllowed(personId,kind){if(personId===self())return false;const row=sharing.find(x=>String(x.person_id)===String(personId)&&x.plan_kind===kind);const cloud=K.supabaseConnection?.state?.authStatus==='authenticated'||K.memberAccess?.state?.status==='authenticated';return row?.allow_view===true&&row?.allow_copy===true;}
...
async function openTeamOverview(value){
 owner=self();try{sharing=await K.supabaseConnection?.readPlanSharing?.()||sharing;K.planSharing=sharing}catch(_){/* lokale Testansicht */}date=value&&K.days.some(d=>d.date===value)?value:(K.days.find(d=>d.type==='market')||K.days[0])?.date;
 if(!date)return;
