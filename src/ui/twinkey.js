...
function entryMethods(){
 const id=self(),stored=K.memberUxData?.colleagueSharing?.[id]?.allow===true;
 shell('Bevor du Zeiten einträgst, entscheidest du selbst, ob Kollegen sie als Vorlage nutzen dürfen.',`<h1>Sollen Kollegen deine verfügbaren Zeiten sehen dürfen?</h1><label class="sw-check"><input type="checkbox" id="twShareWithColleagues" ${stored?'checked':''}> Meine verfügbaren Zeiten dürfen anderen Mitgliedern als Vorlage angezeigt und von ihnen übernommen werden.</label><p><small>Freiwillig. Ohne Haken sehen Kollegen bei deinem Namen nur ein Schloss. Sie können deine Zeiten weder ansehen noch übernehmen.</small></p><div id="twShareStatus" role="status"></div><div class="wa-actions"><button class="ux-btn secondary" id="twShareBack">← Zurück</button><button class="ux-btn primary" id="twShareContinue">Weiter zur Zeiteingabe</button></div>`,'entry');
 $('twShareBack').onclick=tasks;$('twShareContinue').onclick=async()=>{const b=$('twShareContinue'),allow=$('twShareWithColleagues').checked,preferences={can:{view:allow,copy:allow},wish:{view:allow,copy:allow},standby:{view:allow,copy:allow}};b.disabled=true;try{K.memberUxData=K.memberUxData||{};K.memberUxData.colleagueSharing=K.memberUxData.colleagueSharing||{};K.memberUxData.colleagueSharing[id]={allow,updatedAt:new Date().toISOString()};if(K.supabaseConnection?.savePlanSharing&&(K.supabaseConnection.state?.authStatus==='authenticated'||K.memberAccess?.state?.status==='authenticated')){const saved=await K.supabaseConnection.savePlanSharing(preferences);K.planSharing=Array.isArray(K.planSharing)?K.planSharing.filter(r=>String(r.person_id)!==String(id)):[];K.planSharing.push(...saved);}await K.persistAll?.();entryMethodChoices();}catch(e){$('twShareStatus').textContent='Die Einstellung konnte nicht gespeichert werden: '+e.message;b.disabled=false;}};
}
