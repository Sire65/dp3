(function(){
  ...
  function showAvailable(m){currentManifest=m;const currentBuild=Number(window.KC_DP_BUILD||0),nextBuild=Number(m.build||0);shell(`${brand()}<h2>Neue Version verfügbar</h2><p>Möchten Sie <b>KC DP2 V${esc(m.version)}${nextBuild?` · Build ${nextBuild}`:''}</b> jetzt installieren?</p><div class="kc-update-version"><span>Installiert <b>V${esc(K.updateManager.CURRENT_RELEASE)}${currentBuild?` · Build ${currentBuild}`:''}</b></span><span>Neu <b>V${esc(m.version)}${nextBuild?` · Build ${nextBuild}`:''}</b></span></div><div class="kc-update-notes">${notes(m)}</div><p class="kc-update-report-note">Ihre Dienstplandaten bleiben erhalten. Die neue Version wird vor der Aktivierung vollständig geladen und geprüft; bei einem Fehler bleibt die bisherige Version aktiv.</p><div class="kc-update-actions"><button class="kc-update-btn" id="kcUpdLater">Nein, später</button><button class="kc-update-btn primary" id="kcUpdNow">Ja, installieren</button></div>`);$('kcUpdLater').onclick=()=>{K.updateManager.snooze(m.version);close();};$('kcUpdNow').onclick=()=>startInstall(m);}
