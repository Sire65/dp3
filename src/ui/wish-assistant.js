...
function currentSummary(){
 ...
 return `<h2>Deine Auswahl · ${dt(state.date)}</h2><p>Rahmenzeit ${tm(day().start)}–${tm(day().end)} Uhr</p>${K.mobileMatrixUi.summary(rows)}<small>${esc(standbyText(state.standby))}</small><p class="wa-draft-status">${dirty?'Noch nicht gespeichert':'Bisher gespeicherte Angaben'}</p>`;
}
function updateSummary(){const el=$('waCurrentSummary');if(el)el.innerHTML=currentSummary();const demand=$('waDemandLive');if(demand)demand.innerHTML=K.wishDemandUi?.live(rowsFor(state),[state.date])||'';}
