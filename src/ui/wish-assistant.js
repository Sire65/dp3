...
function standbyFor(personId,date){return M().rows(personId,date).filter(w=>w.assistantDay?.standby).sort((a,b)=>String(b.updatedAt||'').localeCompare(String(a.updatedAt||'')))[0]?.assistantDay.standby||K.memberUxData?.assistantStandby?.[personId]?.[date]||{answer:null,slots:[]};}
function standbyText(s){if(!s?.answer)return 'Bereitschaft noch nicht angegeben';return s.answer==='no'?'Keine Bereitschaft':`Bereitschaft zum Einspringen: ${(s.slots||[]).map(x=>tm(x.start)+'–'+tm(x.end)+(x.reserve?' (nur wenn nötig)':'')).join(', ')} Uhr · noch keine Einteilung`;}
...
function bindStaffing(){
 ...
 b.onclick=()=>{
  ...
  box.innerHTML='<b>'+tm(g.start)+'–'+tm(g.end)+' Uhr · '+(g.wishZone==='V'?'Vorne':g.wishZone==='H'?'Hinten':'Beides (V/H)')+'</b>'+
...
K.wishAssistant={openDraft(date,list){open(date);state.can=clone(list.filter(w=>['available','if_needed'].includes(w.wishType)));state.pref=clone(list.filter(w=>w.wishType==='preferred'));state.blocks=clone(list.filter(w=>w.wishType==='unavailable'&&w.scope!=='day'));state.status=list.some(w=>w.wishType==='unavailable'&&w.scope==='day')?'no':state.can.length?'yes':null;state.wishAnswer=state.pref.length?'custom':'none';state.blockAnswer=state.blocks.length?'yes':'no';dirty=true;step=state.status==='no'?'availability':state.can.length?'blocks':'availability';render();},openSuggestion(g){open(g.date);state.pref.push({start:g.start,end:g.end,wishType:"preferred",wishZone:g.zone==="front"?"V":g.zone==="back"?"H":"B"});state.wishAnswer="custom";dirty=true;step="review";render();},start,open,rowsFor,errorsFor,standbyFor,standbyText,standbyHtml};
})();
