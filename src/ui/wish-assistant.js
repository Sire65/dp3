...
function rowsFor(s){
 ...
 return [...s.can,...(s.status==='reserve'||s.wishAnswer==='none'?[]:s.wishAnswer==='all'?s.can.map(w=>({start:w.start,end:w.end,wishType:'preferred',wishZone:w.wishZone,comment:w.comment})):s.pref),...(s.blockAnswer==='no'?[]:s.blocks)].map(w=>({...w,...base,comment:s.applyNote?s.note:w.comment||'',id:w.id||'',date:s.date,scope:'time',wishZone:w.wishType==='unavailable'?'B':s.applyZone?s.zone:w.wishZone||s.zone}));
}
...
function open(date){if(!editable())return start();owner=self();state=load(date);originalAll=clone(M().rows(owner));expected=JSON.stringify(originalAll.filter(w=>w.date===date));step='availability';dirty=false;busy=false;render();}
const sequence=()=>state.status==='no'?['availability','offdays','review']:state.status==='unknown'?['availability','review']:['availability','can',...(state.status==='reserve'?[]:['wish']),'blocks','standby','zone','review'];
...
function readChoice(key){return key==='standby'?state.standby.answer:state[key];}
function slotsHtml(key,slots){return `<div class="wa-slots">${slots.map((x,i)=>`<div class="wa-slot"><div class="wa-timepair">${['start','end'].map(f=>`<label>${f==='start'?'Von':'Bis'}<select data-slot-key="${key}" data-slot-index="${i}" data-slot-field="${f}" aria-label="${key==='can'?'Kann':key==='pref'?'Wunsch':key==='blocks'?'Sperrzeit':'Bereitschaft'} ${i+1} ${f==='start'?'von':'bis'}">${timeOptions(x[f],f)}</select></label>`).join('')}</div><button class="wa-remove" type="button" data-remove="${key}" data-index="${i}">Zeitraum entfernen</button></div>`).join('')}</div><button type="button" class="ux-btn secondary" data-add="${key}">+ Weiterer Zeitraum</button>`;}
function timeOptions(value,field){const nums=new Set(Array.from({length:field==='end'?49:48},(_,i)=>i/2));if(Number.isFinite(value))nums.add(value);return `<option value="">Uhrzeit wählen</option>`+[...nums].sort((a,b)=>a-b).map(n=>`<option value="${n}" ${n===value?'selected':''}>${tm(n)}${n===24?' · Mitternacht':''}</option>`).join('');}
...
function body(){
 const d=day();
 if(step==='availability')return `<h1>Kannst du an diesem Tag helfen?</h1><div class="wa-choices">${choice('status','yes','Ja, ich kann helfen')}${choice('status','reserve','Nur wenn notwendig','Ich helfe, wenn noch Personal fehlt.')}${choice('status','no','Nein, an diesem Tag nicht','Den ganzen Tag sperren.')}${choice('status','unknown','Das weiß ich noch nicht','Den Tag vorerst offen lassen.')}</div>${M().rows(owner,state.date).length?`<details><summary>Meine bisherigen Angaben</summary>${K.mobileMatrixUi.summary(M().rows(owner,state.date))}</details>`:''}`;
 ...
 if(step==='can')return `<h1>Von wann bis wann könntest du helfen?</h1><p>Das ist dein möglicher Zeitraum. Deine Lieblingszeit folgt als Nächstes.</p><div class="wa-presets"><button type="button" class="ux-btn secondary" id="waWhole">Gesamte Rahmenzeit · ${tm(d.start)}–${tm(d.end)}</button><button type="button" class="ux-btn secondary" id="waPrevious">Wie am vorherigen Tag</button></div>${slotsHtml('can',state.can)}${friendHtml()}`;
 if(step==='wish')return `<h1>Welche Zeit wäre dir am liebsten?</h1><p>Du kannst helfen: ${state.can.map(x=>tm(x.start)+'–'+tm(x.end)).join(', ')} Uhr.</p><div class="wa-choices">${choice('wishAnswer','all','Genau diese Zeiten')}${choice('wishAnswer','custom','Ich wünsche mir einen kürzeren Zeitraum')}${choice('wishAnswer','none','Keine besondere Wunschzeit')}</div>${state.wishAnswer==='custom'?slotsHtml('pref',state.pref):''}`;
 if(step==='blocks')return `<h1>Gibt es Zeiten, in denen du nicht kannst?</h1><p>Zum Beispiel wegen eines Termins oder einer Unterbrechung.</p><div class="wa-choices">${choice('blockAnswer','no','Nein, keine Unterbrechung')}${choice('blockAnswer','yes','Ja, ich möchte Zeiten sperren')}</div>${state.blockAnswer==='yes'?slotsHtml('blocks',state.blocks):''}`;
...
function render(){
 ...
 document.querySelectorAll('[data-choice]').forEach(b=>b.onclick=()=>{
  ...
  else{state[key]=value;if(key==='status'&&['yes','reserve'].includes(value)){state.can.forEach(w=>w.wishType=value==='reserve'?'if_needed':'available');if(!state.can.length)state.can=[{start:null,end:null,wishType:value==='reserve'?'if_needed':'available',wishZone:state.zone}];}if(key==='zone')state.applyZone=true;}
  if(key==='wishAnswer'&&value==='all')state.pref=state.can.map(w=>({start:w.start,end:w.end,wishType:'preferred',wishZone:w.wishZone}));
 ...
 document.querySelectorAll('[data-slot-key]').forEach(e=>e.onchange=()=>{dirty=true;const list=e.dataset.slotKey==='standby'?state.standby.slots:state[e.dataset.slotKey];list[Number(e.dataset.slotIndex)][e.dataset.slotField]=e.value===''?null:Number(e.value)});
 ...
 if($('waNote'))$('waNote').oninput=e=>{dirty=true;state.applyNote=true;state.note=e.target.value};
...
async function save(){
 ...
 try{
  ...
  await M().save(owner,dates,rows,baseline);dirty=false;
  const next=K.days[K.days.findIndex(d=>d.date===state.date)+1],pending=K.sync?.snapshot?.()?.outbox?.filter(x=>x.status!=='sent').length;
  shell(`<section class="wa-question wa-done"><span class="wa-eyebrow">GESCHAFFT</span><h1>Deine Angaben sind gespeichert.</h1><p>${dates.length===1?dt(state.date):dates.length+' Sperrtage'}</p>${K.mobileMatrixUi.summary(rows)}<p>${esc(standbyText(rows[0]?.assistantDay?.standby))}</p><p>${pending?'Die Übertragung zum gemeinsamen Wunschplan steht noch aus. Sie läuft über die normale Synchronisierung.':'Die Angaben wurden im Programm gespeichert. Der Verbindungsstatus zeigt die Übertragung zum gemeinsamen Wunschplan.'}</p><div class="wa-actions">${next?'<button class="ux-btn primary" id="waNextDay">Nächsten Tag ausfüllen</button>':''}<button class="ux-btn primary" id="waFinish">Fertig? Meine Auswertung</button><button class="ux-btn secondary" id="waDone">Zur Tagesübersicht</button></div></section>`);
  if($('waNextDay'))$('waNextDay').onclick=()=>open(next.date);$('waDone').onclick=()=>start();$('waFinish').onclick=()=>K.chefCompanion.finish();
