...
begin
  ...
  loop
    ...
    v_checksum := md5(concat_ws('|', p_org_id, trim(p_event_id), v_source_id, v_row::text));
    ...
    insert into public.kc_dp_plan_published (
      ...
      'published', 'dp2', 'KC_DP_PLAN_PUBLISHED_V1/0.2.1',
      v_row, v_checksum, now(), now()
    )
    ...
    where public.kc_dp_plan_published.status is distinct from 'published'
       or public.kc_dp_plan_published.checksum is distinct from excluded.checksum;

    get diagnostics v_changed = row_count;
    v_upserted := v_upserted + v_changed;
  end loop;
  ...
  where org_id = p_org_id
    ...
    and (
      ...
      or not (source_shift_id = any(v_ids))
    );
