-- KC DP2 -> PC-Manager Dienstplan-Bruecke
-- 23.09.2026: Ein Publish-Aufruf ist ein vollstaendiger Snapshot fuer org_id + event_id.
-- Dadurch muss auch ein LEERER Plan alle zuvor noch als published markierten Zeilen entfernen.
-- Gleichzeitig werden bei nicht-leeren Snapshots alte Schichten ausserhalb des neuen Min/Max-
-- Datumsbereichs nicht mehr versehentlich stehen gelassen.
...
begin
  ...
  -- Snapshot-Semantik:
  -- Alles, was fuer dieselbe Organisation + Veranstaltung bisher noch published ist,
  -- aber NICHT mehr im jetzt uebergebenen Snapshot steckt, wird entfernt.
  -- Bei p_rows = [] ist v_ids leer; damit werden bewusst ALLE bisherigen published-Zeilen
  -- dieser Veranstaltung auf removed gesetzt.
...
comment on function public.kc_dp_plan_publish(text, text, jsonb)
is 'Publiziert den vollstaendigen DP2-Sollplan-Snapshot je Organisation und Veranstaltung; fehlende Zeilen werden auf removed gesetzt, auch bei leerem Snapshot.';
