-- Stabilisiert den Quell-Planstand: identische Auto-Sync-Snapshots duerfen published_at/
-- updated_at nicht kuenstlich verjuengen. Nur fachlich veraenderte oder zuvor entfernte Zeilen
-- werden aktualisiert. Die restliche Snapshot-Semantik bleibt identisch.
...
begin
  ...
  for v_row in select value from jsonb_array_elements(p_rows) loop
    ...
    on conflict (org_id, event_id, source_shift_id) do update set
      ...
      updated_at = now()
    where public.kc_dp_plan_published.status <> 'published'
       or public.kc_dp_plan_published.checksum is distinct from excluded.checksum
       or public.kc_dp_plan_published.person_id is distinct from excluded.person_id
       or public.kc_dp_plan_published.work_date is distinct from excluded.work_date
       or public.kc_dp_plan_published.start_time is distinct from excluded.start_time
       or public.kc_dp_plan_published.end_time is distinct from excluded.end_time
       or public.kc_dp_plan_published.break_minutes is distinct from excluded.break_minutes
       or public.kc_dp_plan_published.zone is distinct from excluded.zone
       or public.kc_dp_plan_published.area is distinct from excluded.area;
