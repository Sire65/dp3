...
await reset();await page.locator('#swDayBlock').check();assert(await page.locator('fieldset select').first().isDisabled());assert(await page.locator('#swTimeBlock').isDisabled());await page.locator('#swNext').click();await page.locator('#swNext').click();assert.equal(await page.evaluate(()=>KCDP.wishes[0].scope),'day');
await reset();await page.locator('#swTimeBlock').check();await fill('blocks',0,14,15);await page.locator('#swNext').click();await fill('can',0,12,18);await page.locator('#swNext').click();assert.match(await page.locator('#swError').innerText(),/14:00–15:00/);
await fill('can',0,12,14);await page.locator('#swAddTime').click();await fill('can',1,16,18);await page.locator('#swNext').click();await page.locator('#swSame').click();await page.locator('#swNext').click();await page.locator('#swNext').click();
assert.equal(await page.evaluate(()=>KCDP.wishes.filter(w=>w.wishType==='preferred').length),2);
await reset();await page.locator('#swNext').click();await fill('can',0,11,18);await page.locator('#swNext').click();await page.locator('#swAddTime').click();await fill('times',0,12,14);await page.locator('[data-zone="0"]').selectOption('V');
